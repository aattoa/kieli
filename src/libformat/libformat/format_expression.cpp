...
namespace {
    struct Expression_format_visitor {
        ...
        auto operator()(cst::expression::Conditional const& conditional) {
            ...
            if (!conditional.false_branch.has_value()) return;
            if (auto const* const else_conditional = std::get_if<cst::expression::Conditional>(&conditional.false_branch->body->value)) {
