...
auto libformat::State::format(cst::Qualified_name const& name) -> void {
    if (name.root_qualifier.has_value()) {
        utl::match(name.root_qualifier->value,
            [&](cst::Root_qualifier::Global) {
                format("global::");
            },
...
auto libformat::State::format(cst::expression::Struct_initializer::Member_initializer const& initializer) -> void {
    format("{} = ", initializer.name);
    format(initializer.expression);
}

auto libformat::State::format_mutability_with_trailing_whitespace(tl::optional<cst::Mutability> const& mutability) -> void {
    if (!mutability.has_value()) return;
    format(*mutability);
    format(" ");
}
