...
namespace {
    auto reify_function(libreify::Context& context, hir::Function const& function) -> cir::Function
    {
        auto const reify_parameter_type = utl::compose(
            std::bind_front(&libreify::Context::reify_type, &context),
            &hir::Function_parameter::type);
        return cir::Function {
            .symbol = std::string(
                function.signature.name.identifier.view()), // TODO: format function signature
...
auto kieli::reify(Resolve_result&& resolve_result) -> Reify_result
{
    ...
    std::vector<cir::Function> functions
        = utl::map(std::bind_front(&reify_function, std::ref(context)), resolve_result.functions);
