...
namespace {
        ...
        -> utl::Pair<std::variant<utl::Isize, utl::Usize>>
    {
        ...
        case i16:
            return range(utl::type<utl::I16>);
        case i32:
            return range(utl::type<utl::I32>);
        case i64:
            return range(utl::type<utl::I64>);
        case u8:
            return range(utl::type<utl::U8>);
        case u16:
            return range(utl::type<utl::U16>);
        case u32:
            return range(utl::type<utl::U32>);
        case u64:
            return range(utl::type<utl::U64>);
        default:
            utl::unreachable();
        }
    }
    ...
    struct Expression_lowering_visitor {
        ...
        {
            return std::visit(
                Expression_lowering_visitor { diagnostics, node_arena, expression },
                expression.value);
        }

        auto recurse(utl::Wrapper<cir::Expression> const expression)
            -> utl::Wrapper<lir::Expression>
        {
        ...
        auto operator()(kieli::Integer const& integer_literal) -> lir::Expression
        {
            ...
            try {
                ...
                diagnostics.emit_error({
                    ...
                    .message
                    = "The value of this integer literal is outside of the valid range for {}"_format(
                        cir::to_string(this_expression.type)),
                    .help_note = "The valid range for {} is {}..{}"_format(
                        cir::to_string(this_expression.type), min, max),
                });
            }
        }
