...
TEST("integer literal suffix") {
    ...
    SECTION("erroneous suffix after exponent") {
        REQUIRE_THAT(lex_failure("5e3wasd"), contains("erroneous integer literal alphabetic suffix"));
    }
}

TEST("integer literal valid range") {
    REQUIRE(lex_success(fmt::format("{}", std::numeric_limits<utl::Usize>::max())) ==
        fmt::format("(int: '{}'), end of input", std::numeric_limits<utl::Usize>::max()));
...
TEST("integer literal digit separators") {
    ...
    REQUIRE_THAT(lex_failure("1'"), contains("expected one or more digits after the digit separator"));
    REQUIRE_THAT(lex_failure("0x'"), contains("expected one or more digits after the base-16 specifier"));
}
