...
TEST("floating point literal explicit base rejection") {
    REQUIRE_THAT(lex_failure("0x0.0"), contains("a floating point literal may not have a base specifier"));
}
...
TEST("floating point literal suffix") {
    SECTION("erroneous suffix") {
        REQUIRE_THAT(lex_failure("5.0wasd"), contains("erroneous floating point literal alphabetic suffix"));
    }
    ...
    SECTION("erroneous suffix after exponent") {
        REQUIRE_THAT(lex_failure("5.0e3wasd"), contains("erroneous floating point literal alphabetic suffix"));
    }
}
