...
utl::Pooled_string::Pooled_string(utl::Usize const index, utl::Usize const size, std::string const* const pool)
...
auto utl::String_pool::make(std::string_view const string) -> Pooled_string {
    ...
    if (it == m_string->end())
        ...
        return Pooled_string { unsigned_distance(m_string->begin(), it), string.size(), m_string.get() };
}
