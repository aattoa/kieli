...
namespace {
    ...
    auto parse_impl(std::string_view const string, std::same_as<int> auto const... base)
        -> std::expected<T, liblex2::Numeric_error>
    {
        ...
        if (ptr != end) {
            return std::unexpected { liblex2::Numeric_error::invalid_argument };
        }
        if (ec != std::errc {}) {
            cpputil::always_assert(ec == std::errc::result_out_of_range);
            return std::unexpected { liblex2::Numeric_error::out_of_range };
        }
        return value;
    }
} // namespace

auto liblex2::apply_scientific_exponent(std::size_t integer, std::size_t const exponent)
...
auto liblex2::parse_integer(std::string_view const digits, int const base)
    -> std::expected<std::size_t, Numeric_error>
{
    return parse_impl<std::size_t>(without_separators(digits), base);
}

auto liblex2::parse_floating(std::string_view const digits) -> std::expected<double, Numeric_error>
{
