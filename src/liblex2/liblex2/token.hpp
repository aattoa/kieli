...
template <>
struct std::formatter<kieli::Token2::Type> : std::formatter<std::string_view> {
    auto format(kieli::Token2::Type const type, auto& context) const
    {
        return std::formatter<std::string_view>::format(kieli::Token2::type_string(type), context);
    }
};

template <>
struct std::formatter<kieli::Token2> : std::formatter<std::string_view> {
    auto format(kieli::Token2 const& token, auto& context) const
    {
        if (std::holds_alternative<std::monostate>(token.value)) {
            return std::formatter<std::string_view>::format(
                kieli::Token2::type_string(token.type), context);
        }
