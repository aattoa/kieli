...
static_assert(std::is_trivially_copyable_v<kieli::Token2>);
...
auto kieli::Token2::description(Type const type) noexcept -> std::string_view
{
    // clang-format off
    switch (type) {
...
auto kieli::Token2::type_string(kieli::Token2::Type const type) noexcept -> std::string_view
{
    static_assert(type_strings_array.size() == utl::enumerator_count<kieli::Token2::Type>);
