        ...
        std::span<utl::Pooled_string const> const imports) -> void
    {
        for (utl::Pooled_string const import : imports) {
            ...
            auto parse_result = kieli::parse(kieli::lex({
                ...
                .source           = compilation_info.get()->source_arena.wrap(
                    utl::Source::read(std::filesystem::path { path })),
            }));
            read_modules_to(
                module_map, source_directory, compilation_info, parse_result.module.imports);
...
auto compiler::compile(Compile_arguments&& compile_arguments) -> Compilation_result
{
    ...
    combined_desugar_result.module.definitions.reserve(ranges::fold_left(
    ...
    for (auto& [_, desugar_result] : module_map) { // NOLINT
        ...
        utl::append_vector(
            combined_desugar_result.module.definitions,
            std::move(desugar_result.module.definitions));
    }
