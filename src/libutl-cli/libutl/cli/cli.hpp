...
namespace cli {
    ...
    inline auto integer(std::string_view const name = {}) -> Value<types::Int>
    {
        return { name };
    }

    inline auto floating(std::string_view const name = {}) -> Value<types::Float>
    {
        return { name };
    }

    inline auto boolean(std::string_view const name = {}) -> Value<types::Bool>
    {
        return { name };
    }

    inline auto string(std::string_view const name = {}) -> Value<types::Str>
    {
        return { name };
    }
    ...
    struct [[nodiscard]] Named_argument {
        using Variant = std::variant<types::Int, types::Float, types::Bool, types::Str>;

        std::string name; // short-form names are automatically converted to long-form, which is
                          // also why the string must be an owning one
        std::vector<Variant> values;
    };
    ...
    [[nodiscard]] auto to_string(cli::Options_description const&) -> std::string;

    [[nodiscard]] auto
    parse_command_line(int argc, char const* const* argv, Options_description const&)
