...
#define REQUIRE_RESOLUTION_FAILURE(expression, error_string) \
    REQUIRE_THROWS_MATCHES(                                  \
        (void)(expression),                                  \
        std::exception,                                      \
        Catch::Matchers::MessageMatches(contains(error_string)))
...
TEST("name resolution")
{
    ...
    REQUIRE_RESOLUTION_FAILURE(
        resolve("namespace test {} fn f() = test::f()"),
        "test does not contain a definition for 'f'");
    REQUIRE_RESOLUTION_FAILURE(
        resolve("fn f() = global::g()"),
        "the global namespace does not contain a definition for 'g'");
    REQUIRE(
        resolve("namespace a {"
                "namespace b { fn f() = g() }"
                "fn g() = 5: I64"
        ...
        == "fn f(): I64 = ({ (g()): I64 }): I64"
           "fn g(): I64 = ({ (5): I64 }): I64");
    REQUIRE(
        resolve("namespace test { fn f(): I32 = ??? } fn f() = (test::f(), ())")
        == "fn f(): I32 = ({ (\?\?\?): I32 }): I32"
...
{
    REQUIRE_THAT(
        resolution_diagnostics("fn f() { let x = \?\?\?; }"), contains("unused local variable"));
    REQUIRE_THAT(
        resolution_diagnostics("fn f() { let x = \?\?\?; let x = \?\?\?; }"),
        contains("shadows an unused local variable"));
    REQUIRE(
        resolve("fn f() {"
                "let x = 3.14;"
                "let x = \"hello\";"
...
{
    REQUIRE_RESOLUTION_FAILURE(
        resolve("fn f(x: *I32): I32 { dereference(x) }"), "may not appear within safe context");
    REQUIRE(
        resolve("fn f(x: *I32): I32 { unsafe { dereference(x) } }")
...
TEST("mutability")
{
    ...
    REQUIRE_RESOLUTION_FAILURE(
        resolve("fn f() { let x = ' '; &mut x }"), "acquire mutable reference");
    REQUIRE_RESOLUTION_FAILURE(
        resolve("fn f[m: mut]() { let mut?m x = ' '; &mut x }"), "acquire mutable reference");
    REQUIRE(
        resolve("fn f() {"
                "let x = 3.14;"
                "let y = &x;"
    ...
    REQUIRE(
        resolve("fn f() { let a = \?\?\?; let b: &I32 = &(*a); b }")
        == "fn f(): &immut I32 = ({ (let immut a: &immut I32 = (\?\?\?): &immut I32): (); (let "
           "immut b: &immut I32 = (&immut (*(a): &immut I32): I32): &immut I32): (); (b): &immut "
           "I32 }): &immut I32");
    REQUIRE(
        resolve("fn f() { let a = \?\?\?; let b: &mut I32 = &mut (*a); b }")
        == "fn f(): &mut I32 = ({ (let immut a: &mut I32 = (\?\?\?): &mut I32): (); (let immut b: "
           "&mut I32 = (&mut (*(a): &mut I32): I32): &mut I32): (); (b): &mut I32 }): &mut I32");
    REQUIRE(
        resolve("fn f() { let a = \?\?\?; let b = &mut *a; let b: Char = *b; b }")
        == "fn f(): Char = ({ (let immut a: &mut Char = (\?\?\?): &mut Char): (); (let immut b: "
           "&mut Char = (&mut (*(a): &mut Char): Char): &mut Char): (); (let immut b: Char = "
           "(*(b): &mut Char): Char): (); (b): Char }): Char");
}
...
{
    REQUIRE(
        resolve("fn f() { let x = ???; let f: fn(String): I64 = \?\?\?; f(x) }")
        == "fn f(): I64 = ({ "
...
{
    REQUIRE(
        resolve("fn f(): Char { let x = \?\?\?; unsafe { dereference(addressof(x)) } }")
        == "fn f(): Char = ({ "
...
{
    REQUIRE(
        resolve("fn f() { let mut x: U8 = 5; let _: &mut U8 = &mut x; }")
        == "fn f(): () = ({ (let mut x: U8 = (5): U8): (); (let _: &mut U8 = (&mut (x): U8): &mut "
           "U8): (); (()): () }): ()");
    REQUIRE(
        resolve("fn f() { let mut x: U8 = 5; let _: &U8 = &mut x; }")
        == "fn f(): () = ({ (let mut x: U8 = (5): U8): (); (let _: &immut U8 = (&mut (x): U8): "
           "&mut U8): (); (()): () }): ()");
}
...
{
    REQUIRE_RESOLUTION_FAILURE(
        resolve("fn f() { let x = 5; let _: (I32, I64) = (x, x); }"),
        "the explicitly specified type is");
    REQUIRE_RESOLUTION_FAILURE(
        resolve("fn f() { let x = \?\?\?; let _: (String, I8) = (x, x); }"),
        "the explicitly specified type is");
}
...
{
    REQUIRE(
        resolve("struct S = a: I32, b: I64 fn f() = S { a = 10, b = 5 }")
        == "fn f(): S = ({ (S { (10): I32, (5): I64 }): S }): S");
    REQUIRE(
        resolve("struct S = a: I32, b: I64 fn f() = S { b = 10, a = 5 }")
        == "fn f(): S = ({ (S { (5): I32, (10): I64 }): S }): S");
    REQUIRE_RESOLUTION_FAILURE(
        resolve("struct S = a: I32, b: I64 fn f() = S { a = ' ' }"), "initializer is of type Char");
    REQUIRE_RESOLUTION_FAILURE(
        resolve("struct S = a: I32, b: I64 fn f() = S { a = 10 }"), "'b' is not initialized");
    REQUIRE_RESOLUTION_FAILURE(
        resolve("struct S = a: I32, b: I64 fn f() = S { b = 10 }"), "'a' is not initialized");
    REQUIRE_RESOLUTION_FAILURE(
        resolve("struct S = a: I32, b: I64 fn f() = S { a = 0, b = 0, c = 0 }"), "S does not have");
}
...
TEST("loop resolution")
{
    ...
    REQUIRE_RESOLUTION_FAILURE(
        resolve("fn f() { loop { break \"\"; break 5; } }"),
        "previous break expressions had results of type String");
    REQUIRE_THAT(
        resolution_diagnostics("fn f() = while true {}"),
        contains("'loop' instead of 'while true'"));
...
{
    REQUIRE_RESOLUTION_FAILURE(
        resolve("fn f[T](): T = ??? fn g() = f[]()"),
        "requires exactly 1 template argument, but 0 were supplied");
    REQUIRE_RESOLUTION_FAILURE(
        resolve("fn f[A, B](): (A, B) = ??? fn g() = f[I8]()"),
        "requires exactly 2 template arguments, but 1 was supplied");
    REQUIRE_RESOLUTION_FAILURE(
        resolve("fn f[A, B](): (A, B) = ??? fn g() = f[I8, I16, I32]()"),
        "requires exactly 2 template arguments, but 3 were supplied");
    REQUIRE_RESOLUTION_FAILURE(
        resolve("fn f[A, B=I64](): (A, B) = ??? fn g() = f[I8, I16, I32]()"),
        "has only 2 template parameters, but 3 template arguments were supplied");
    REQUIRE_RESOLUTION_FAILURE(
        resolve("fn f[A, B, C=B](): (A, B, C) = ??? fn g() = f[I8]()"),
        "requires at least 2 template arguments, but 1 was supplied");
    REQUIRE_RESOLUTION_SUCCESS(resolve("fn f[A, B=I64](): (A, B) = ??? fn g() = f[I8]()"));
    REQUIRE_RESOLUTION_SUCCESS(resolve("fn f[A, B=A](): (A, B) = ??? fn g() = f[I8]()"));
    REQUIRE_RESOLUTION_SUCCESS(resolve("namespace test { struct S = s: I64 fn f[A, B=S](): (A, B) "
                                       "= ??? } fn g() = test::f[I8]()"));
}
...
{
    REQUIRE(
        resolve("fn f[T](c: Bool, a: T, b: T) = if c { a } else { b }"
        ...
        == "fn g(): Float = ({ "
           ...
           " }): I32");
    REQUIRE(
        resolve("enum Option[T] = none | some(T) fn f() { let _: Option[I32] = "
                "Option[I32]::some(5: I32) }")
...
{
    REQUIRE(
        resolve("struct Triple[A, B = A, C = B] = a: A, b: B, c: C "
    ...
    REQUIRE(
        ...
        == "fn f(): Triple[I32, String, String] = ({ "
           "(Triple[I32, String, String] { (\?\?\?): I32, (\?\?\?): String, (\?\?\?): String }): "
...
TEST("wildcard template arguments")
{
    REQUIRE(
        ...
        == "fn f(): S[String, Float] = ({ "
           ...
           " }): S[Float, String]");
    REQUIRE(
        resolve("struct S[A = _, B = _] = a: A, b: B "
                "fn f() = S[] { a = \"aaa\", b = 2.74 } "
...
TEST("map option")
{
    REQUIRE(
        ...
        == "fn f(immut o: Option[I32]): String = ({ "
           "(get[String]((map[I32, String]((o): Option[I32], (\?\?\?): fn(I32): String)): "
...
TEST("function generalization")
{
    ...
    REQUIRE(
        resolve("fn f(x: _, y: typeof(x)) = (x, y)"
                "fn g() = f(\?\?\?, 3.14)")
        == "fn g(): (Float, Float) = ({ "
