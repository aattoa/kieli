...
namespace {
    struct Pattern_resolution_visitor {
        ...
        auto operator()(ast::pattern::Tuple& tuple) -> hir::Pattern
        {
            ...
            bool const is_exhaustive = ranges::all_of(
                hir_tuple_pattern.field_patterns, &hir::Pattern::is_exhaustive_by_itself);
        ...
        auto operator()(ast::pattern::Alias& alias) -> hir::Pattern
        {
            ...
            hir::Mutability const mutability
                = context.resolve_mutability(alias.alias_mutability, scope);
            ...
            tl::optional<utl::Wrapper<ast::Pattern>> const ast_payload_pattern) -> hir::Pattern
        {
            ...
            if (ast_payload_pattern.has_value()) {
                if (constructor.payload_type.has_value()) {
                    hir_payload_pattern
                        = recurse(*utl::get(ast_payload_pattern), *constructor.payload_type);
                }
                else {
                    context.error(
                        this_pattern.source_view,
                        { .message = std::format(
                              "Constructor '{}' has no fields to be handled", constructor.name) });
                }
            }
                ...
                context.error(
                    this_pattern.source_view,
                    { .message = std::format(
                          "Constructor '{}' has fields which must be handled", constructor.name) });
            }

            bool const is_exhaustive
                = (!hir_payload_pattern || hir_payload_pattern->is_exhaustive_by_itself)
               && utl::get<hir::type::Enumeration>(*constructor.enum_type.flattened_value())
                          .info->constructor_count()
                      == 1;
        ...
        {
            return utl::match(
                context.find_lower(ast_constructor.constructor_name, scope, space),
                [&](utl::Wrapper<Function_info>) -> hir::Pattern {
                    context.error(
                        this_pattern.source_view,
                        { "Expected a constructor, but found a function" });
                },
                [&](utl::Wrapper<Namespace>) -> hir::Pattern {
                    context.error(
                        this_pattern.source_view,
                        { "Expected a constructor, but found a namespace" });
                },
        ...
        {
            if (auto const* const enumeration_ptr
                = std::get_if<hir::type::Enumeration>(&*matched_type.flattened_value()))
            {
                hir::Enum& enumeration = context.resolve_enum(enumeration_ptr->info);
                auto const it          = ranges::find(
                    enumeration.constructors,
                    ast_constructor.constructor_name,
                    &hir::Enum_constructor::name);
                if (it == enumeration.constructors.end()) {
                    context.error(
                        ast_constructor.constructor_name.source_view,
                        {
                            .message = std::format(
            ...
            else if (std::holds_alternative<hir::type::Unification_variable>(
                         *matched_type.pure_value()))
            {
                context.error(
                    this_pattern.source_view,
                    {
                        .message = "Abbreviated constructor pattern used with an unsolved "
                                   "unification type variable",
                        .help_note
                        = "This can likely be solved by providing additional type annotations, "
        ...
        auto operator()(ast::pattern::Slice& slice) -> hir::Pattern
        {
            hir::Type const element_type
                = context.fresh_general_unification_type_variable(this_pattern.source_view);
...
auto libresolve::Context::resolve_pattern(
    ast::Pattern& pattern, hir::Type const type, Scope& scope, Namespace& space) -> hir::Pattern
{
    return std::visit(
        Pattern_resolution_visitor { *this, scope, space, pattern, type }, pattern.value);
}
