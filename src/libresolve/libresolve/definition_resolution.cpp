...
namespace {
        ...
        ast::definition::Function&& function) -> void
    {
        ...
        if (has_explicit_return_type) {
            ...
            hir::Expression function_body = context.resolve_expression(function.body, signature_scope, *function_info.home_namespace);
...
auto libresolve::Context::resolve_struct(utl::Wrapper<Struct_info> const wrapped_info) -> hir::Struct& {
...
auto libresolve::Context::resolve_typeclass(utl::Wrapper<Typeclass_info> const wrapped_info) -> hir::Typeclass& {
    ...
    if (auto* const ast_typeclass = std::get_if<ast::definition::Typeclass>(&info.value)) {
        ...
        auto handle_signature = [&](ast::Function_signature&& signature) {
            ...
            hir_typeclass.function_signatures.add_new_or_abort(identifier, std::move(hir_signature));
        };
...
auto libresolve::Context::resolve_implementation(utl::Wrapper<Implementation_info> const wrapped_info) -> hir::Implementation& {
...
auto libresolve::Context::resolve_instantiation(utl::Wrapper<Instantiation_info> const wrapped_info) -> hir::Instantiation& {
    (void)wrapped_info;
    utl::todo();
}
...
auto libresolve::Context::resolve_implementation_template(utl::Wrapper<Implementation_template_info>) -> hir::Implementation_template& {
    utl::todo();
}


auto libresolve::Context::resolve_instantiation_template(utl::Wrapper<Instantiation_template_info>) -> hir::Instantiation_template& {
    utl::todo();
}
