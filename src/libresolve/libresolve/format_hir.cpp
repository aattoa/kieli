...
template <>
struct std::formatter<hir::Unification_type_variable_state::Unsolved> : utl::formatting::Formatter_base {
    auto format(hir::Unification_type_variable_state::Unsolved const& unsolved, auto& context) const {
        std::format_to(
            ...
            std::invoke([kind=unsolved.kind] {
                switch (kind) {
                case hir::Unification_type_variable_kind::general: return 'T';
                case hir::Unification_type_variable_kind::integral: return 'I';
                default: utl::unreachable();
                }
            }),
            unsolved.tag.value);
        if (!unsolved.classes.empty()) {
            std::format_to(context.out(), ": {}", utl::formatting::delimited_range(unsolved.classes, " + "));
        }
        return context.out();
    }
};
    ...
    struct Expression_format_visitor {
        ...
        auto operator()(hir::expression::Function_reference const& function) {
            ...
            if (function.info->template_instantiation_info.has_value())
                std::format_to(out, "[{}]", function.info->template_instantiation_info->template_arguments);
        }
        ...
        auto operator()(hir::expression::Match const& match) {
            std::format_to(out, "match {} {{ {} }}", match.matched_expression, utl::formatting::delimited_range(match.cases, " "));
        }
        ...
        auto operator()(hir::expression::Enum_constructor_reference const& reference) {
            std::format_to(out, "{}::{}", reference.constructor.enum_type, reference.constructor.name);
        }
...
DEFINE_FORMATTER(hir::Mutability) {
    utl::match(*value.flattened_value(),
        ...
        [&](hir::Mutability::Parameterized const& parameterized) {
            std::format_to(context.out(), "mut?{}'{}", parameterized.tag.value, parameterized.identifier);
        },
