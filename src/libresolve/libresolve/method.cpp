...
namespace {
    ...
    struct [[nodiscard]] Method_lookup_result {
        ...
        std::variant<utl::Wrapper<Implementation_info>, utl::Wrapper<Implementation_template_info>>
            implementation_info;
    };
        ...
        -> Method_lookup_result
    {
        ...
        for (utl::wrapper auto const implementation_info :
             context.nameless_entities.implementations)
        {
            hir::Implementation& implementation
                = context.resolve_implementation(implementation_info);
            hir::Implementation::Definitions& definitions = implementation.definitions;

            // Try to find a method with the given name first and only then check if the
            // implementation concerns the given type, because the former is a much cheaper
            // operation than the latter.

            if (utl::wrapper auto* const function
                = definitions.functions.find(method_name.identifier))
            {
        ...
        context.error(
            method_name.source_view, { "No appropriate method '{}' in scope"_format(method_name) });
    }
    ...
    Namespace& space) -> utl::Wrapper<Function_info>
{
    ...
    if (template_arguments.has_value()) {
        return instantiate_function_template(
            lookup_result.method_info, *template_arguments, method_name.source_view, scope, space);
    }

    if (resolve_function_signature(*lookup_result.method_info).is_template()) {
        return instantiate_function_template_with_synthetic_arguments(
            lookup_result.method_info, method_name.source_view);
    }
    else {
