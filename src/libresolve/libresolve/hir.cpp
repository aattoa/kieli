...
hir::Mutability::Mutability(utl::Wrapper<Variant> const value, utl::Source_view const source_view) noexcept
    : m_value { value }, m_source_view { source_view } {}
...
auto hir::Unification_type_variable_state::as_unsolved(std::source_location const caller) noexcept -> Unsolved& {
    return utl::get<Unsolved>(m_value, caller);
}
auto hir::Unification_type_variable_state::as_unsolved(std::source_location const caller) const noexcept -> Unsolved const& {
    return utl::get<Unsolved>(m_value, caller);
}
...
hir::Unification_mutability_variable_state::Unification_mutability_variable_state(Unsolved&& unsolved) noexcept
    : m_value { std::move(unsolved) } {}
...
auto hir::Unification_mutability_variable_state::as_unsolved(std::source_location const caller) noexcept -> Unsolved& {
    return utl::get<Unsolved>(m_value, caller);
}
auto hir::Unification_mutability_variable_state::as_unsolved(std::source_location const caller) const noexcept -> Unsolved const& {
    return utl::get<Unsolved>(m_value, caller);
}
