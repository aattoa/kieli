...
namespace {
    ...
    struct [[nodiscard]] Substitutions {
        ...
        auto add_substitution(
            hir::Template_parameter const& parameter, hir::Template_argument const& argument)
            -> void
        {
            std::visit(
                utl::Overload {
    ...
    struct Substitution_context {
        ...
        [[nodiscard]] auto recurse(instantiable auto const& node) const
        {
        ...
        [[nodiscard]] auto recurse() const noexcept
        {
            ...
            if (actual_argument_count != minimum_argument_count) {
                if (maximum_argument_count == 0) {
                    context.error(
                        instantiation_view,
                        { .message = std::format(
                              "{} has no explicit template parameters, but {} explicit template {} "
                              "supplied",
                              template_name,
        ...
        return std::visit(
            utl::Overload {
                [&](hir::Template_parameter::Type_parameter const& type_parameter,
                    utl::Wrapper<ast::Type> const                  type_argument) {
                ...
                [&](hir::Template_parameter::Type_parameter const& type_parameter,
                    ...
                    utl::wrapper auto const state = context.fresh_unification_type_variable_state(
        ...
        utl::Source_view const                         instantiation_view) -> void
    {
        ...
        for (auto const& [parameter, argument] : ranges::views::zip(parameters, arguments)) {
            output_arguments.push_back(resolve_single_template_argument(
                context, scope, space, parameter, argument, instantiation_view));
        }
    }
        ...
        utl::Source_view const                         instantiation_view) -> void
    {
        ...
        for (hir::Template_parameter const& parameter : parameters) {
            ...
            hir::Template_argument default_argument
                = substitution_context.recurse(resolve_single_template_argument(
                    context,
                    parameter.default_argument->scope ? *parameter.default_argument->scope
                                                      : empty_scope,
                    template_space,
                    parameter,
    ...
    {
        auto const first_defaulted = ranges::find_if(parameters, [](auto const& parameter) {
            return parameter.default_argument.has_value();
        });
        auto const first_implicit
            = ranges::find_if(parameters, &hir::Template_parameter::is_implicit);

        utl::Usize const minimum_required_argument_count
            = utl::unsigned_distance(parameters.begin(), first_defaulted);
        utl::Usize const maximum_permitted_argument_count
        ...
        utl::Usize const defaulted_argument_count
            = utl::unsigned_distance(first_defaulted, parameters.end());
        utl::always_assert(
            parameters.size() == minimum_required_argument_count + defaulted_argument_count);
        ...
        Namespace&                            space) -> utl::Wrapper<Function_info>
    {
        ...
        hir::Type const concrete_function_type {
            resolution_context.wrap_type(hir::type::Function {
                .parameter_types
                = utl::map(&hir::Function_parameter::type, concrete_function_parameters),
        ...
        Namespace&                               space) -> utl::Wrapper<Struct_info>
    {
        ...
        for (hir::Struct::Member const& member : struct_template.definition.members) {
            concrete_struct.members.push_back(hir::Struct::Member {
                .name      = member.name,
        ...
        auto const concrete_info    = resolution_context.wrap(Struct_info {
               ...
               .template_instantiation_info = Template_instantiation_info<Struct_template_info> {
                template_info, struct_template.parameters, std::move(template_arguments) } });
        ...
        Namespace&                             space) -> utl::Wrapper<Enum_info>
    {
        ...
        hir::Enum concrete_enum {
            ...
            = resolution_context.wrap(Namespace { .parent = template_info->home_namespace })
        };
        ...
        for (hir::Enum_constructor& constructor : enum_template.definition.constructors) {
            hir::Enum_constructor concrete_constructor {
                ...
                .function_type
                = constructor.function_type.transform([&](hir::Type const function_type) {
                      return hir::Type {
                          resolution_context.wrap_type(hir::type::Function {
                              .parameter_types = utl::map(
                                  substitution_context.recurse(),
                                  utl::get<hir::type::Function>(*function_type.flattened_value())
                                      .parameter_types),
            ...
            concrete_enum.associated_namespace->lower_table.add_new_or_abort(
                concrete_constructor.name.identifier, concrete_constructor);
        }
    ...
    struct Expression_instantiation_visitor {
        ...
        auto operator()(hir::expression::Block const& block) -> R
        {
            return hir::expression::Block {
                ...
                = utl::map(context.recurse(), block.side_effect_expressions),
        ...
        template <class T>
            requires compiler::literal<T>
                  || utl::one_of<
                         ...
                         hir::expression::Local_variable_reference,
                         hir::expression::Hole>
    ...
    struct Type_instantiation_visitor {
        ...
        auto operator()(hir::type::Structure const& structure) -> R
        {
            if (structure.is_application) {
                auto& instantiation_info = utl::get(structure.info->template_instantiation_info);

                return context.resolution_context.wrap_type(hir::type::Structure {
                    .info = instantiate_struct_template_application(
                        context.resolution_context,
                        context.resolution_context.resolve_struct_template(
                            instantiation_info.template_instantiated_from),
                        instantiation_info.template_instantiated_from,
                        utl::map(context.recurse(), instantiation_info.template_arguments),
                        context.scope,
        ...
        auto operator()(hir::type::Enumeration const& enumeration) -> R
        {
            if (enumeration.is_application) {
                auto& instantiation_info = utl::get(enumeration.info->template_instantiation_info);

                return context.resolution_context.wrap_type(hir::type::Enumeration {
                    .info = instantiate_enum_template_application(
                        context.resolution_context,
                        context.resolution_context.resolve_enum_template(
                            instantiation_info.template_instantiated_from),
                        instantiation_info.template_instantiated_from,
                        utl::map(context.recurse(), instantiation_info.template_arguments),
                        context.scope,
    ...
    struct Pattern_instantiation_visitor {
        ...
        {
            hir::Type const enum_type = context.recurse(pattern.constructor.enum_type);

            hir::Enum& enumeration = context.resolution_context.resolve_enum(
                utl::get<hir::type::Enumeration>(*enum_type.pure_value()).info);

            auto it = ranges::find(
                enumeration.constructors, pattern.constructor.name, &hir::Enum_constructor::name);
    ...
    auto instantiate(hir::Pattern const& pattern, Substitution_context const context)
        -> hir::Pattern
    {
        return {
    ...
    auto instantiate(hir::Mutability const& mutability, Substitution_context const context)
        -> hir::Mutability
    {
        if (auto const* parameterized
            = std::get_if<hir::Mutability::Parameterized>(&*mutability.flattened_value()))
        {
    ...
    auto instantiate(hir::Template_argument const& argument, Substitution_context const context)
        -> hir::Template_argument
    {
        return hir::Template_argument {
    ...
    [[nodiscard]] auto synthetize_arguments_for(
        std::span<hir::Template_parameter const> const parameters,
        utl::Source_view const argument_view) -> std::vector<ast::Template_argument>
    {
        return std::vector(
...
auto libresolve::Context::instantiate_function_template_with_synthetic_arguments(
    utl::Wrapper<Function_info> const template_info, utl::Source_view const instantiation_view)
    -> utl::Wrapper<Function_info>
{
    auto const arguments = synthetize_arguments_for(
        resolve_function(template_info).signature.template_parameters, instantiation_view);
    Scope instantiation_scope;
    return instantiate_function_template(
        template_info,
        arguments,
        instantiation_view,
        instantiation_scope,
...
{
    auto const arguments = synthetize_arguments_for(
        resolve_struct_template(template_info).parameters, instantiation_view);
    Scope instantiation_scope;
    return instantiate_struct_template(
        template_info,
        arguments,
        instantiation_view,
        instantiation_scope,
...
auto libresolve::Context::instantiate_enum_template_with_synthetic_arguments(
    utl::Wrapper<Enum_template_info> const template_info, utl::Source_view const instantiation_view)
    -> utl::Wrapper<Enum_info>
{
    auto const arguments = synthetize_arguments_for(
        resolve_enum_template(template_info).parameters, instantiation_view);
    Scope instantiation_scope;
    return instantiate_enum_template(
        template_info,
        arguments,
        instantiation_view,
        instantiation_scope,
...
{
    auto const arguments = synthetize_arguments_for(
        resolve_alias_template(template_info).parameters, instantiation_view);
    Scope instantiation_scope;
    return instantiate_alias_template(
        template_info,
        arguments,
        instantiation_view,
        instantiation_scope,
